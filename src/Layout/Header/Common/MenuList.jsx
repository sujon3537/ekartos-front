   ...
   return (
      <>
         <li className={`${customClass ? customClass : ''} ${menu?.badge ? 'new-nav-item' : ''} ${menu.children ? 'dropdown-mega' : ''}`}>
            {menu?.path ? (
               <Link className={`${anchorClass ? anchorClass : 'nav-link dropdown-toggle'}`} href={{ pathname: `${menu?.path}`, query: menu?.params }}>
                  {t(menu?.title)}
               </Link>
            ) : (
               <>
               ...
               <div className={`dropdown-menu dropdown-menu-2 dropdown-image ${!isOpen.length ? 'show' : isOpen[level] === menu.title ? 'show' : ''}`}>
                  <div className='dropdown-column'>
                     {menu?.children?.map((data, i) => (
                        <Link className={'dropdown-item'} href={{ pathname: `${data?.path}`, query: data?.params }} key={i}>
                              ...
                              {elem?.column?.map((head, i) => (
                                 <Fragment key={i}>
                                    {head?.type == 'sub' ? (
                                       ...
                                       <Link className={'dropdown-item'} href={{ pathname: `/${head?.path}`, query: head?.params }}>
                                          {t(head?.title)}
                                          {head?.label && <label className={`menu-label ${head?.labelClass ?? ''}`}>{head?.label}</label>}
                                       </Link>
                                    )}
